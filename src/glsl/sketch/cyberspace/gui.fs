...
void main() {
  vec2 uv1 = ((vec3(vUv - 0.5, 1.0) * rotateMat2D(time)).xy + 0.5);
  vec2 uv2 = ((vec3(vUv - 0.5, 1.0) * rotateMat2D(-time * 0.75)).xy + 0.5);
  vec2 uv3 = ((vec3(vUv - 0.5, 1.0) * rotateMat2D(-time * 0.5)).xy + 0.5);
