...
void main(void) {
  ...
  float texColorR = texture2D(noiseTex, (v.yz + vec2(v.x, 0.0) + time * multiTime * 4.0) * 0.004).r;
  float texColorG = texture2D(noiseTex, (v.zx + vec2(v.y, 0.0) + time * multiTime * 4.0) * 0.004).g;
  float texColorB = texture2D(noiseTex, (v.xy + vec2(v.z, 0.0) + time * multiTime * 4.0) * 0.004).b;
  vec3 noise = vec3(
    texColorR * 2.0 - 1.0,
    texColorG * 2.0 - 1.0,
    texColorB * 2.0 - 1.0
  ...
  vec3 f2 = (f + a + d + af * 0.002) * (1.0 - init) + af * init;
