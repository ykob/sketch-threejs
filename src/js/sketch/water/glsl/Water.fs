...
void main() {
  ...
  vec3 normal = (modelMatrix * vec4(normalize(normal1 + normal2 + normal3 + normal4), 1.0)).xyz * (1.0 - smoothstep(0.4, 1.0, length(p)));
  ...
  gl_FragColor = vec4(rgb - diffuse + pow(diffuse, 20.0), 1.0) + tSceneColor;
}
